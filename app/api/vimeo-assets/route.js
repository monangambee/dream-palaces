...
export async function GET() {
  try {
    ...
    const customGif = textData[0]?.fields?.ScreeningGif || "";
      ...
      client.request(
        {
          ...
          query: {
            per_page: 100,
            fields:
              "uri,name,description,duration,created_time,pictures,link,privacy,stats",
          },
        },
        (error, body) => {
          if (error) {
          ...
          const assets = body.data.map((video) => {
            ...
            return {
              ...
              aspectRatio:
                video.pictures?.sizes?.[0]?.width /
                  video.pictures?.sizes?.[0]?.height || 16 / 9,
